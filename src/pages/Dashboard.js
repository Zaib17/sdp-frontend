...
const Dashboard = () => {
  ...
  const [meters, setMeters] = useState({
    ...
    house1: { meterId: "A-002", name: "Syed Hassan", power: 0 },
    ...
    const fetchMeters = async () => {
      try {
        ...
        const updatedMeters = {
          ...
          house1: { ...house1, units: calcUnits("A-002", house1.power) },
  ...
  const houseTotal =
    (meters.house1?.power || 0) +
    (meters.house2?.power || 0) +
    (meters.house3?.power || 0);

  const powerLoss = Math.abs(
    (meters.streetInput?.power || 0) -
      (meters.toNext?.power || 0) -
      houseTotal
  );
